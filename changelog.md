...
- 2026-04-03 `stdHTML` WIP - Added `QuerySelector` and `QuerySelectorAll` to `src/WIP/stdHTML.cls` with support for tag/id/class/`[attr=value]`, grouped selectors, and combinators (` `, `>`, `+`, `~`).
- 2026-04-03 `stdHTML` WIP - Added `CreateFromHTML(html As String)` in `src/WIP/stdHTML.cls` with a native VBA parser (tags/attributes/text, void/self-closing support, SVG passthrough raw blocks), plus parser-option scaffolding/TODO hooks for boolean attributes and comment preservation.
